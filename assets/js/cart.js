...
const SHIP_COST           = { delivery: 100, cvs: 60, store: 0 };
...
function updateCartUI() {
  ...
  // Totals
  const method   = document.querySelector('input[name="shipping"]:checked')?.value || 'delivery';
  const shipCost = sub >= FREE_SHIP_THRESHOLD ? 0 : SHIP_COST[method];
  const total    = sub + shipCost;

  document.getElementById('cartSubtotal').textContent = `NT$ ${sub.toLocaleString()}`;
  document.getElementById('cartShipping').textContent = shipCost === 0
    ? t('shop.cart.free') : `NT$ ${shipCost}`;
  ...
  if (sub >= FREE_SHIP_THRESHOLD) {
    ...
    const diff = FREE_SHIP_THRESHOLD - sub;
    note.textContent = t('shop.cart.free-note').replace('{diff}', diff.toLocaleString());
...
async function loadProducts() {
  const res  = await fetch('menu_2.json');
  const data = await res.json();
  _products  = data.menu['咖啡豆零售 Coffee Beans Retail'].items;
  renderGrid();
}
...
function renderCard(product, lang) {
  ...
  const roastLabel  = isEn
    ? (product.roast_level_zh === '淺焙'       ? 'Light'
     : product.roast_level_zh === '淺中焙'     ? 'Light-Medium'
     : product.roast_level_zh === '中淺焙'     ? 'Medium-Light'
     : product.roast_level_zh === '中焙'       ? 'Medium'
     : 'Med-Dark')
  ...
  const addLabel    = product.in_stock ? `<i class="fas fa-plus"></i> ${t('shop.product.add')}` : t('shop.product.sold-out');

  const grindOptions = product.variants.map(v => {
...
function wireCardEvents() {
  ...
  grid.querySelectorAll('.grind-radio').forEach(radio => {
    radio.addEventListener('change', e => {
      ...
      if (variant) {
        card.querySelector('.product-card__price').textContent = `NT$ ${variant.price_NTD.toLocaleString()}`;
